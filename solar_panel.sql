...
drop procedure if exists logme;
drop procedure if exists givecontactbymail;
DELIMITER $$
create procedure logme(date_creationi date ,titrei char(255) , messagei char(255))
  BEGIN
  insert into messages (date_creation,titre,message)
    values(date_creationi, titrei,messagei);
  END $$
  DELIMITER ;

  DELIMITER $$
create procedure givecontactbymail(emaili char(255))
  BEGIN
  select nom,prenom,telephone from contacts where emaili=email;
  END $$
  DELIMITER ;
-- --------------------------------------------------------
